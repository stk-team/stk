...
template <unsigned char N>
PDGenerator<N>::PDGenerator(bool p) :
    _c(0), _v(0.0), _i(0UL)
{
...
template <unsigned char N>
void
PDGenerator<N>::spherical_boundary()
{
...
template <unsigned char N>
void
PDGenerator<N>::box_boundary(const Tuple<N,float>& bound)
{
...
template <unsigned char N>
inline double
PDGenerator<N>::volume() const
{ return _v; }

template <unsigned char N>
inline typename PDGenerator<N>::Iterator
PDGenerator<N>::begin() const
{ return Iterator(_t->begin()); }

template <unsigned char N>
inline typename PDGenerator<N>::Iterator
PDGenerator<N>::end() const
{ return Iterator(_t->end()); }

template <unsigned char N>
inline
PDGenerator<N>::Iterator::Iterator() {}

template <unsigned char N>
inline
PDGenerator<N>::Iterator::Iterator(const typename Tree<N>::Iterator& i) :
    Tree<N>::Iterator(i) {}

template <unsigned char N>
inline bool
PDGenerator<N>::Iterator::operator != (const Iterator& i) const
...
template <unsigned char N>
void
PDGenerator<N>::output(stk::PointSet2dd& pts) const
{
    Cube<N> cube;
