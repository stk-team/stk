...
int main(int argc, char* argv[]){
    ...
    if(argc != 3){
        std::cerr << "Parameters: samplingPattern numTrials" << std::endl;
        exit(-2);
    }
    ...
    ss.str(std::string());
    ss << resultFolder << "variance-sphericalcap-from-powspec-" << samplingpattern << "/";
    ...
    double* powSpecSphericalCap = new double[nlmax] ();
    //int nTrials=10;

    //##########################################################

    std::ofstream fvarpowspec;
    ss.str(std::string());
    ss << datafiles << "variance-sphericalcap-from-powspec-" << samplingpattern << "-l" << nlmax << "-trial-" << nTrials << ".txt";
    fvarpowspec.open(ss.str().c_str());

    for(int Nside=1; Nside < 30; Nside++){

        int N = 12 * Nside * Nside;

        //Initializing to zero all buffers
        for(int r = 0; r < nlmax; r++){
            powSpecAccum[r] = 0;
            powSpecSphericalCap[r] = 0;
            t_powSpec[r] = 0;
        ...
        for(int trial=1; trial<= nTrials; trial++){
            fprintf(stderr," Trials (%7d) : N (%7d)",trial,N);
        ...
        //##########################################################
        // Compute variance of the spherical cap using Power spectrum
        // of the sampling pattern and the Spherical Cap itself;

        powspec_spherical_cap(powSpecSphericalCap,30.0,nlmax);
        double variance = 0;
        for(int l=0; l < nlmax; l++){
            variance +=   (1.0/(2l+1)*(2l+1)) * powSpecSphericalCap[l] * (powSpecAccum[l] * (1.0 / double(nTrials)));
        }
        fvarpowspec << N <<" " << variance * (4*PI*PI*PI/double(N)) << std::endl;
    }
    fvarpowspec.close();
    //##########################################################
    std::cerr << std::endl;
